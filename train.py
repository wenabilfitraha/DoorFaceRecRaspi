...
if __name__ == '__main__':
    ...
    for filename in walk_files(setup.POSITIVE_DIR, '*.pgm'):
        faces.append(prepare_image(filename))
        labels.append(setup.POSITIVE_LABEL)
        pos_count += 1
    # Read all negative images
    for filename in walk_files(setup.NEGATIVE_DIR, '*.pgm'):
        faces.append(prepare_image(filename))
        labels.append(setup.NEGATIVE_LABEL)
        neg_count += 1
    ...
    # Save model results
    model.save(setup.TRAINING_FILE)
    print('Training data saved to', setup.TRAINING_FILE)
